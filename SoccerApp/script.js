...
/* ------------------------------------------------------ Display current user account ----------------------------------------------------  */
    ...
    function displayCurrentProfile() {
        ...
        fetch('api/api.php?action=displayCurrentProfile',
        {
            ...
            if(response.status === 202) {
                data.forEach(row => {
                    displayFName = row.FirstName
                    displayLName = row.LastName
                    displayEmail = row.Email
                    displayPhone = row.Phone
                    displayAddress = row.Address
                })
                document.getElementById("editFName").value = displayFName ;
                document.getElementById("editLName").value = displayLName ;
                document.getElementById("editEmail").value = displayEmail;
                document.getElementById("editPhone").value = displayPhone;
                document.getElementById("editAddress").value = displayAddress;
            } else {
